...
from nn.ssd2.ssd_layers import PriorBox, Normalize
...
def SSD(input_shape, num_classes=21):
    ...
    conv1_1 = Conv2D(64, (3, 3),activation='relu',padding='same',name='conv1_1')(input_img)
    ...
    # FC7
    fc7 = Conv2D(1024, (1, 1), padding='same', activation='relu',name='fc7')(fc6)
    ...
    conv5_3_norm = Normalize((num_classes-1)/2,name='conv_5_3_norm')(conv_deconv5_3)
    EltSUM=add([conv4_3_norm,conv5_3_norm])
    fusion_conv4_5=Activation(name='fusion_conv4_5',activation='relu')(EltSUM)
    num_priors = 3
    fusion_conv4_5_mbox_loc = Conv2D(num_priors * 4, (3, 3), padding='same',name='fusion_conv4_5_mbox_loc')(fusion_conv4_5)
    fusion_conv4_5_mbox_loc_flat =  Flatten(name='fusion_conv4_5_mbox_loc_flat')(fusion_conv4_5_mbox_loc)
    name = 'fusion_conv4_5_mbox_conf'
    if num_classes != 21:
        name += '_{}'.format(num_classes)
    fusion_conv4_5_mbox_conf = Conv2D(num_priors * num_classes, (3, 3), padding='same',name=name)(fusion_conv4_5)
    fusion_conv4_5_mbox_conf_flat = Flatten(name='fusion_conv4_5_mbox_conf_flat')(fusion_conv4_5_mbox_conf)
    fusion_conv4_5_mbox_priorbox = PriorBox(img_size, 30.0, aspect_ratios=[2],variances=[0.1, 0.1, 0.2, 0.2],name='fusion_conv4_5_mbox_priorbox')(fusion_conv4_5)
    # Prediction from fc7
    num_priors = 6
    ...
    mbox_loc = concatenate([fusion_conv4_5_mbox_loc_flat,fc7_mbox_loc_flat,conv6_2_mbox_loc_flat,conv7_2_mbox_loc_flat,conv8_2_mbox_loc_flat,pool6_mbox_loc_flat],axis=1, name='mbox_loc')
    mbox_conf = concatenate([fusion_conv4_5_mbox_conf_flat,fc7_mbox_conf_flat,conv6_2_mbox_conf_flat,conv7_2_mbox_conf_flat,conv8_2_mbox_conf_flat,pool6_mbox_conf_flat],axis=1, name='mbox_conf')
