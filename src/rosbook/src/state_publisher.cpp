...
int main(int argc, char** argv) {
    ...
    // robot state
    double inc = 0.005, base_arm_inc = 0.005, arm1_armbase_inc = 0.005, arm2_arm1_inc = 0.005, gripper_inc = 0.005, tip_inc = 0.005, angle = 0, base_arm = 0, arm1_armbase = 0, arm2_arm1 = 0, gripper = 0, tip = 0;
    ...
    while (ros::ok()) {
        ...
        joint_state.name[1] = "arm1_to_arm_base";
        joint_state.position[1] = arm1_armbase;

        joint_state.name[2] = "arm2_to_arm1";
        ...
        if (arm2_arm1 < -1.5 || arm2_arm1 > 1.5)
            arm2_arm1_inc *= -1;
        if (arm1_armbase > 1.2 || arm1_armbase < -1)
            arm1_armbase *= -1;
        if (base_arm > 1 || base_arm < -1)
            base_arm_inc *= -1;
